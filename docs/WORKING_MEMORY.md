...
- Favoritos globales nativos: `Ctrl+P` / `Ctrl+Shift+P` recorren `flash` (`deepseek-v4-flash:low`), `pro` (`deepseek-v4-pro:high`) y `pro-max` (`deepseek-v4-pro:max`). `Alt+M` abre `/models`; en Roles, `Enter` reasigna modelo, `t` ajusta effort, `c` agrega/quita del ciclo y `[`/`]` reordena.
