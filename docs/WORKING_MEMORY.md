...
- Perfil visual global: `display.hideToolActivity: false` mantiene visibles las llamadas/resultados de tools y `terminal.showProgress: true` publica progreso nativo mientras el agente o el mantenimiento de contexto siguen activos. `Ctrl+Shift+O` sólo puede ocultar filas aún repintables: el scrollback nativo ya comprometido por WezTerm es inmutable. El wrapper global `extensions/windows-input.ts` registra `Ctrl+Shift+M` aun si WinInput nativo no carga; abre una vista fullscreen reversible de la rama actual sin tool calls/results y cerrarla restaura intactos transcript, scrollback y selección.
