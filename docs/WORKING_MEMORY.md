...
- Perfil visual global: `Ctrl+T` controla thinking y `Ctrl+Shift+O` toda la actividad de tools. El patch `patches/omp-17.4.0-workstation.patch` agrega `display.hiddenTools`, `display.hideAssistantToolPreambles` y la API de extensión para reconstruir el transcript principal mediante `resetDisplay()`. `Ctrl+Shift+M` ya no abre otra vista: WezTerm emite el chord privado `Ctrl+Alt+O` y `extensions/tool-activity-view.ts` muestra un selector modal para thinking, preámbulos, toggle global y cada tool registrada. Al cerrarlo se conserva el transcript nativo con Markdown, renderers, streaming y editor. Estado inicial validado: thinking, preámbulos y `bash` ocultos.
- Barra de estado global ajustada: tema `dark-poimandres-compact`, effort textual (`min`, `low`, `med`, `hgh`, `xhi`, `max`), ruta completa, `context_pct` sin `context_total` y `usage` nativo sin footer paralelo. El status relee el cache actualizado por headers Codex al terminar mensajes assistant, limitado a una vez por minuto, y filtra estándar/Spark según el modelo activo; el fetch completo conserva cinco minutos. `omp.exe` sirve el build 17.4 parcheado y carga el addon existente desde el sidecar `~/.bun/bin/pi_natives.win32-x64-baseline.node`. WezTerm lo lanza por ruta absoluta; sesiones ya abiertas conservan el core anterior y reciben como máximo un aviso para abrir un tab nuevo. `omp.com` permanece bloqueado por la sesión origen hasta su reemplazo limpio desde el artifact staged.
