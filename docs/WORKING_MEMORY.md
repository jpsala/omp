...
- Perfil visual global: `display.hideToolActivity: false` mantiene visibles las llamadas/resultados de tools y `terminal.showProgress: true` publica progreso nativo mientras el agente o el mantenimiento de contexto siguen activos. `Ctrl+Shift+O` sólo puede ocultar filas aún repintables: el scrollback nativo ya comprometido por WezTerm es inmutable. El wrapper global `extensions/windows-input.ts` registra el chord interno `Ctrl+Alt+O`; Main consume `Ctrl+Shift+M` dentro de WezTerm y lo rerutea a ese chord para abrir la vista fullscreen reversible sin tool calls/results. Cerrarla restaura intactos transcript, scrollback y selección.
