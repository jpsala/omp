...
- Perfil visual global: `display.hideToolActivity: false` mantiene visibles las llamadas/resultados de tools y `terminal.showProgress: true` publica progreso nativo mientras el agente o el mantenimiento de contexto siguen activos. `Ctrl+Shift+O` sólo puede ocultar filas aún repintables: el scrollback nativo ya comprometido por WezTerm es inmutable. `Ctrl+Shift+M` abre una vista fullscreen reversible de la rama actual sin tool calls/results; cerrarla restaura intactos transcript, scrollback y selección.
