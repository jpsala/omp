...
- Perfil visual global sobre OMP 18.0.4: `Ctrl+T` controla thinking y `Ctrl+Shift+O` toda la actividad de tools. El patch `patches/omp-18.0.4-workstation.patch` agrega `display.hiddenTools`, `display.hideAssistantToolPreambles` y la API de extensión para reconstruir el transcript principal mediante `resetDisplay()`. Todos los `TranscriptContainer`, incluido el staging que `renderInitialMessages()` intercambia durante startup/replay, reciben la política granular. `Ctrl+Shift+M` abre el selector modal sin abandonar el transcript nativo. Estado verificado end-to-end: thinking, preámbulos y `bash` marcados; una llamada Bash real ejecutó `TRULY_HIDDEN` sin renderizar call ni output y dejó visible sólo la respuesta final.
