...
- Perfil visual global: `display.hideToolActivity: false` mantiene visibles las llamadas/resultados de tools y `terminal.showProgress: true` publica progreso nativo mientras el agente o el mantenimiento de contexto siguen activos. `Ctrl+Shift+O` sólo puede ocultar filas aún repintables: el scrollback nativo ya comprometido por WezTerm es inmutable. El wrapper global `extensions/windows-input.ts` registra el chord interno `Ctrl+Alt+O`; WezTerm reemplaza su `Hide` default de `Ctrl+Shift+M` y emite directamente ese chord privado para abrir una vista fullscreen reversible sólo de conversación: mensajes del usuario y respuestas finales, sin thinking, preámbulos de tools, tool calls ni resultados. Main no participa. Cerrarla restaura intactos transcript, scrollback y selección.
- Barra de estado global ajustada: tema `dark-poimandres-compact`, effort textual (`min`, `low`, `med`, `hgh`, `xhi`, `max`), ruta completa, `context_pct` sin `context_total` y `usage` nativo sin footer paralelo. El status relee el cache actualizado por headers Codex al terminar mensajes assistant, limitado a una vez por minuto, y filtra estándar/Spark según el modelo activo; el fetch completo conserva cinco minutos. El rollout global de prueba sirve el source parcheado desde `~/.bun/bin/omp.exe`; `omp.com` queda staged hasta que la sesión anterior libere su lock.
