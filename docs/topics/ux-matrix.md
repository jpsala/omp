...
| Orca `1.4.197` | Host visual con worktrees, editor, terminales, Chat UI nativa, browser, Computer Use y lifecycle/resume específico para OMP. | **Nativa como host del proceso OMP**: inyecta una extensión de status mediante `ORCA_OMP_STATUS_EXTENSION`, reconoce eventos, approvals, session id y resume por transcript absoluto. El piloto verificó setup-before-agent, Chat UI, input, paste UTF-8, copy Markdown, scroll dirigido, apertura de archivos, estado `done` y resume manual. | Piloto visual activo. Usar Orca para superficie, worktrees y terminales; conservar Task/Hub, Habitat y Fleet como autoridad de ejecución/orquestación salvo pedido explícito de Orca orchestration. `Agent sleep` queda experimental hasta verificar su resume automático en Chat UI. |
