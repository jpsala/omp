...
| Orca `1.4.197` | Interfaz visual con worktrees, rich editor, terminales, sidebar `Agent Session History`, Source Control, Checks, Computer Use, Mobile y lifecycle/resume específico para OMP. La conversación viva permanece en el TUI; el sidebar estructura el transcript, últimos turnos, log y acciones de resume, pero no reemplaza el terminal por una Chat UI. | **Nativa como host del proceso OMP**: inyecta `ORCA_OMP_STATUS_EXTENSION`, reconoce eventos, approvals, `ask`, session id y resume por transcript absoluto. La adopción verificó setup-before-agent, UTF-8, copy/scroll del TUI, archivos, rich Markdown, tabs/splits, diff, branch publish, atención, Task/Hub real, pairing móvil, reinicio con contexto, resume de esta sesión y handoff a WezTerm. El indicador puede conservar un `Working` obsoleto aunque el turno haya terminado. | Interfaz visual principal reversible como workspace host, no como chat client. OMP conserva ejecución y tools; Task/Hub, Habitat y Fleet conservan sus autoridades. Mantener `Agent sleep` apagado y no programar automations OMP mientras el estado final y la captura de output/usage no sean confiables. |
