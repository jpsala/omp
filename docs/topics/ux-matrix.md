...
| Orca `1.4.197` | Interfaz visual con worktrees, rich editor, terminales, Chat UI nativa, sidebar `Agent Session History`, Source Control, Checks, Computer Use, Mobile y lifecycle/resume específico para OMP. El item integrado `+ → OMP` abre la Chat UI estructurada de Orca; el Quick Command global `OMP TUI`, visible como botón directo en la barra de tabs, ejecuta `omp` y abre el TUI real sin pasar por PowerShell. | **Dos superficies distintas**: la integración nativa usa OMP como backend de Chat; el Quick Command hospeda el proceso TUI completo con su perfil, tools y gates. El status hook instalado ya liquida `agent_end.willContinue !== true`, pero el generador de título de `1.4.197` todavía espera `ctx.isIdle()` y puede dejar su timer vivo. `bun run fix:orca-spinner` aplica un reemplazo binario de igual longitud sobre el seam único del bundle y corrige también la extensión activa; falla cerrado ante otra forma de fuente. | Usar `OMP TUI` como ingreso diario de un clic. Reservar `+ → OMP` para la Chat UI nativa cuando se quiera explícitamente esa superficie. OMP conserva ejecución y tools; Task/Hub, Habitat y Fleet conservan sus autoridades. Reaplicar el workaround tras una actualización sólo si upstream todavía genera la rama vulnerable; retirarlo cuando el contrato sea nativo. `Agent sleep` permanece apagado hasta una decisión explícita y automations OMP hasta que capturen output/usage confiable. |
