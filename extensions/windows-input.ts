 ...
 *
 * This file is the durable source. The installed profile copy lives at
 * `~/.omp/agent/extensions/windows-input.ts`.
 *
 ...
 *
 * Load/reload: restart OMP or run `/reload` after updating the installed copy.
 * Toggle: `/windows-input on|off|toggle|status`.
 * Model presets: `Ctrl+Alt+M` cycles default/medium, luna/xhigh, luna/max.
...
class WindowsInputEditor extends CustomEditor {
	...
	private onModelPresetHotkey?: () => void;

	setModelPresetHotkey(handler: (() => void) | undefined): void {
		this.onModelPresetHotkey = handler;
	}
...
export default function (pi: ExtensionAPI) {
	...
	let presetIndex = -1;
	let cyclingPreset = false;

	const apply = (ctx: ExtensionContext) => {
		if (!ctx.hasUI) return;
		const cycleModelPreset = async (): Promise<void> => {
			if (cyclingPreset) return;
			cyclingPreset = true;
			try {
				presetIndex = (presetIndex + 1) % presets.length;
				const preset = presets.at(presetIndex);
				if (!preset) return;
				const model = ctx.models.resolve(preset.model);
				if (!model) {
					ctx.ui.notify(`No se pudo resolver ${preset.model}`, "error");
					return;
				}
				if (!(await pi.setModel(model))) {
					ctx.ui.notify(`No se pudo activar ${preset.label}`, "error");
					return;
				}
				pi.setThinkingLevel(preset.thinking);
				ctx.ui.notify(`Preset: ${preset.label}`, "info");
			} finally {
				cyclingPreset = false;
			}
		};
		ctx.ui.setEditorComponent(
			enabled
				? (tui: TUI, theme: EditorTheme, keybindings: KeybindingsManager) => {
						...
						editor.setModelPresetHotkey(() => void cycleModelPreset());
						return editor;
					}
				: undefined,
		);
