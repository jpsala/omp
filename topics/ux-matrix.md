...
| --- | --- | --- | --- |
| Windows input, clipboard y perfiles | **portar sobre renderer nativo** | En 17.2.11, `src/utils/clipboard.ts` implementa lectura PowerShell nativa/WSL de texto e imagen; `src/modes/controllers/input-controller.ts` conecta paste de texto/imagen al editor; `@oh-my-pi/pi-tui/src/terminal.ts` habilita VT input, UTF-8 y mitigaciones ConPTY en `win32`. `extensions/windows-input.ts` carga el `CustomEditor` sólo cuando está disponible el addon opcional; mientras tanto conserva el hotkey y el listener de terminal consume `U+00B5` sin insertar `µ`, por lo que `/`, `@`, `#`, skills e internal URLs siguen en el editor OMP nativo. En el layout Windows activo, WezTerm entrega `Ctrl+Alt+M` como U+00B5; la extensión reconoce tanto el chord parseable como ese carácter y usa `ctx.models.resolve`, `pi.setModel` y `pi.setThinkingLevel` para recorrer los padres de `profiles/catalog.json`. | Mantener la fuente durable en `extensions/windows-input.ts` y el wrapper instalado en el perfil OMP. No volver a copiar ni reemplazar el renderer completo. |
...
| Modelos y esfuerzos rápidos | **native** | `modelRoles` acepta selectores con sufijo de effort; `modelTags` nombra roles; `cycleOrder` define el ciclo de `Ctrl+P`/`Ctrl+Shift+P`; `/models` permite editar Roles y `Alt+M` selecciona el modelo de la sesión. | Usar favoritos nativos para cambios rápidos de modelo+effort; reservar overlays/perfiles para Task, `prewalk` y concurrencia. |
